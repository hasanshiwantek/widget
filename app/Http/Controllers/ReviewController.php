...
class ReviewController extends Controller
{
    ...
    public function reviewsCts(){
        $reviews = Review::query()
        ->where('brand', 3) // Replace 'brand' with the actual column name
        ->get();
        $count = $reviews -> count();
    ...
    public function statsCts(){
        $stats = Stat::query()
        ->where('brand', 3) // Filter records where brand equals 1
        ->latest('updated_at') // Order by updated_at in descending order
        ->first();
        // $count = $stats -> count();
